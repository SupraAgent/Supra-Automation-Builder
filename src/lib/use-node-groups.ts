...
/**
 * Derives locked groups from node data.
 * Group membership is stored in node.data.groupId.
 * This is the single source of truth — no separate state to sync.
 */
export function useNodeGroups(nodes: Node[]) {
  // Derive the group map and ID→node lookup from current nodes
...
/**
 * Intercepts node changes to enforce locked-group constraints:
 * - When one group member is dragged, all members move together
 * - When one group member is selected, all are selected
 * - Individual removal of locked-group nodes is blocked
 */
export function applyGroupDragConstraints(
  ...
  // Build O(1) lookup map once per call
  ...
  for (const change of changes) {
    // Enforce group drag — move all members together
    if (
      ...
      // If this node's group was already handled, skip the duplicate
      if (gid && handledGroupDrags.has(gid)) continue;
    }

    // Enforce group select — select all members together
    ...
    // Block individual removal of locked-group nodes
